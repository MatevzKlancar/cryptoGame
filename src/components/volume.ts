...
export class Volume implements Renderable {
  ...
  public constructor(position: Point, controller: Controller) {
    ...
    this.sprite = new Sprite("img/volume3.png", this.soundButtonDimensions);
  ...
  public Render(renderContext: CanvasRenderingContext2D): Renderable[] {
    ...
    this.opacity = Math.max(
      this.opacity - Volume.opacityDecay,
      Volume.fadedOpacity
    );
    ...
    renderContext.translate(
      this.soundButtonPosition.x,
      this.soundButtonPosition.y
    );
  ...
  private isClickInVolume(click: Point): boolean {
    return (
      click &&
      click.x > this.soundButtonPosition.x &&
      click.x < this.soundButtonPosition.x + this.soundButtonDimensions.x &&
      click.y > this.soundButtonPosition.y &&
      click.y < this.soundButtonPosition.y + this.soundButtonDimensions.y
    );
  }

  private updateAllSounds(): void {
    this.sounds.forEach((sound: Sound) => {
      sound.volume = this.isMuted ? 0 : this.level / 5;
    });
  }
}
